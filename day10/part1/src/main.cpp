    ...
    for (auto& trailhead : trailheads) {
        ...
        do {
            ...
            for (auto &v : visited) {
                ...
                if (x + 1 >= 0 && x + 1 < size_x && y >= 0 && y < size_y && lines[y][x + 1] == current && std::find_if(visited.begin(), visited.end(), [&](std::pair<char, std::pair<int, int>> v) { return v.first == current && v.second == std::pair<int, int>{x + 1, y}; }) == visited.end())
                    visited.push_back({current, {x + 1, y}});
                if (x - 1 >= 0 && x - 1 < size_x && y >= 0 && y < size_y && lines[y][x - 1] == current && std::find_if(visited.begin(), visited.end(), [&](std::pair<char, std::pair<int, int>> v) { return v.first == current && v.second == std::pair<int, int>{x - 1, y}; }) == visited.end())
                    visited.push_back({current, {x - 1, y}});
                if (x >= 0 && x < size_x && y + 1 >= 0 && y + 1 < size_y && lines[y + 1][x] == current && std::find_if(visited.begin(), visited.end(), [&](std::pair<char, std::pair<int, int>> v) { return v.first == current && v.second == std::pair<int, int>{x, y + 1}; }) == visited.end())
