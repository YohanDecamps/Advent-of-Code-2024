    ...
    for (const auto &[key, values] : antennas) {
        for (auto v1 = values.begin(); v1 != values.end(); v1++) {
            for (auto v2 = v1 + 1; v2 != values.end(); v2++) {
                std::pair<int, int> p1 = {(*v1).first + ((*v1).first - (*v2).first), (*v1).second + ((*v1).second - (*v2).second)};
                std::pair<int, int> p2 = {(*v2).first + ((*v2).first - (*v1).first), (*v2).second + ((*v2).second - (*v1).second)};
