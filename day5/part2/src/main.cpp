...
int main() {
    ...
    auto sortCondition = [&rules](int a, int b) {return std::find(rules[a].begin(), rules[a].end(), b) != rules[a].end();};
    ...
    [&sortCondition, &rules](int sum, std::vector<int> update) {
        if (!std::is_sorted(update.begin(), update.end(), sortCondition)) {
            std::nth_element(update.begin(), update.begin() + update.size() / 2, update.end(), sortCondition);
